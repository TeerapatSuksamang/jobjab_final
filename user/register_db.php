<?php
    ...
    if(isset($_POST['submit'])){
        ...
        $select_username = mysqli_query($conn, "SELECT * FROM `user` WHERE `username` = '$username' ");
        
        if($select_username -> num_rows > 0){
            alert('ชื่อผู้ใช้ซ้ำ กรุณาเปลี่ยนใหม่', 'register.php');
        } else {
            if(move_uploaded_file($_FILES['img']['tmp_name'], $filepath)){
                $sql = mysqli_query($conn, "INSERT INTO `user`(`firstname`, `lastname`, `img`, `username`, `password`, `address`, `phone`)
                VALUES('$firstname', '$lastname', '$filename', '$username', '$password', '$address', '$phone') ");
            } else {
                $sql = mysqli_query($conn, "INSERT INTO `user`(`firstname`, `lastname`, `username`, `password`, `address`, `phone`)
