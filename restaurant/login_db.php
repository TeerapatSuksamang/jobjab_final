<?php
    ...
    if(isset($_POST['submit'])){
        ...
        $select = mysqli_query($conn, "SELECT * FROM `restaurant` WHERE `username` = '$username' ");
        if($select -> num_rows > 0){
            $select_user = mysqli_query($conn, "SELECT * FROM `restaurant` WHERE `username` = '$username' AND `password` = '$password' ");
            $row_user = mysqli_fetch_array($select_user);
            if($select_user -> num_rows > 0){
                if($row_user['status'] == 1){
                    session_start();
                    $_SESSION['res_id'] = $row_user['res_id'];
